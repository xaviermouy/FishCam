...
Run once before each deployment (interactively, with sudo):
  ...
  2. Validates the wittypi section of fishcam_config.yaml
  3. Shows a visual 24-hour schedule preview for confirmation
  4. Synchronises the WittyPi RTC from the system clock
  5. Generates and loads the power schedule into WittyPi
  6. Writes voltage protection thresholds and power-cut delay via I2C

Usage:
    sudo python3 configure_wittypi.py
...
def main():
    ...
    _print_check(ok, msg, warn=not ok)  # warning, not hard failure
    ...
    if has_dst:
        _print_check(False,
            ...
            f"schedule will drift by the offset change after transition",
            warn=True)
    else:
    ...
    # Show the schedule file that will be written
    print()
